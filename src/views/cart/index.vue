...
export default {
  ...
  setup () {
    ...
    const changeChecked = (skuId, event) => {
      store.dispatch('cart/updateChange', { skuId, selected: event })
    }
    ...
    const goCheckout = () => {
      if (store.getters['cart/selectedList'].length === 0) {
        return Message({ type: 'warning', text: '请至少选择一件商品' })
      }
      if (!store.state.user.profile.token) {
        return Confirm({ text: '下单功能需要登录，您是否去登录？' })
          .then(() => {
