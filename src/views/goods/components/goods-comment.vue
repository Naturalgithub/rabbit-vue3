...
export default {
  ...
  setup () {
    ...
    const formatNickname = (name) => {
      const fisrtName = name.slice(0, 1)
      const lastName = name.slice(-1)
      const formatNickname = `${fisrtName}****${lastName}`
      return formatNickname
    }
    const formatSpecs = (specs) => {
      const formatSpecs = specs.reduce((prev, item) => {
        prev = `${prev} ${item.name}:${item.nameValue} `
        return prev
      }, '')
      return formatSpecs
    }
